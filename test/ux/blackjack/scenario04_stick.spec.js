...
describe('FEATURE: Blackjack', () => {
  ...
  describe('SCENARIO 05: Stick', () => {
    ...
    it('    AND if the dealer has not won THEN the player is declared the winner', () => { if (!results.playerHasWon) return getElementText(driver, 'results_area').should.eventually.equal('WINNER: Dealer.'); });
    ...
    it('      AND if the player has not won THEN the dealer is declared the winner', () => { if (!results.dealerHasWon) return getElementText(driver, 'results_area').should.eventually.equal('WINNER: Player.'); });
  });
});
